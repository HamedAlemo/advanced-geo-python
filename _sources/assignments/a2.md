...
1. Add Python as a dependency to this project. 
...
A single distance is not very interesting. Let's expand your script to hold a list of three locations (name, latitude, longitude) and print a table of the distances between them. To format that table nicely, add the [`tabulate`](https://github.com/astanin/python-tabulate) package from `conda-forge`:
...
1. Move to a different directory (for example your home directory) and clone a fresh copy of the whole repository under a new name, then run the tool:
   ```
   $ cd ~
